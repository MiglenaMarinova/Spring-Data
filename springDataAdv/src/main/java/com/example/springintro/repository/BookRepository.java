...
public interface BookRepository extends JpaRepository<Book, Long> {
    ...
    List<Book> findBooksByTitleContains(String givenStr);

    List<Book> findBooksByAuthor_LastNameStartingWith(String startStr);

    @Query("SELECT count (b) FROM Book b WHERE length(b.title) > :param")
    int countBookByTitleGreaterThan(@Param(value = "param") int length);
